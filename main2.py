...
tab1, tab2, tab3, tab4 = st.tabs(["Secure-Trust", "Documentação Técnica", "Protótipo", "Sobre Nós"])

with tab1:
    st.header("🏦 Cenário: Projeto SecureTrust")
...
if uploaded_file is not None and processar:
    try:
        ...
        # Thresholds manuais para avaliação tabular
        st.sidebar.subheader("Thresholds para Avaliação Tabular")
        thr_input = st.sidebar.text_input("Ex: 0.3,0.5,0.7", value="0.3,0.5,0.7")
        try:
...
with tab4:
    ...
    **Mentoria/Professores:** [Se quiser mencionar]
    """)

    st.success("Obrigado por acessar nosso projeto!")